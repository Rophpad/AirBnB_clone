...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_show_nonexisting_instance(self):
        """tests show method when an instance is not created yet"""
        console = HBNBCommand()
        prompt = "(hbnb)"
        with patch('builtins.print') as mock_print:
            console.onecmd("show BaseModel 121212")
            mock_print.assert_called_with("** no instance found **")

    def test_show_existing_instance(self):
        """tests show method when an instance is created with
        ...
        with patch('builtins.print') as mock_print:
            ...
            console.onecmd(f'show BaseModel {id}')
            self.assertTrue(id == mock_print.call_args_list[0][0][0].id)

    def test_show_missing_class(self):
        """tests show method when no class is passed"""
        console = HBNBCommand()
        prompt = "(hbnb)"
        with patch("builtins.print") as mock_print:
            console.onecmd("show")
    ...
    def test_show_missing_id(self):
        """tests show method when no id is passed"""
        console = HBNBCommand()
        prompt = "(hbnb)"
        with patch("builtins.print") as mock_print:
    ...
    def test_destroy_nonexisting_instance(self):
        ...
        prompt = "(hbnb)"
        with patch('builtins.print') as mock_print:
            console.onecmd("destroy BaseModel 121212")
            mock_print.assert_called_with("** no instance found **")
    ...
    def test_destroy_existing_instance(self):
        ...
        with patch('builtins.input', return_value=f"yes\n"):
            console.onecmd(f'destroy BaseModel {output}')

        with patch('builtins.print') as mock_print:
            console.onecmd(f"show BaseModel {output}")
            mock_print.assert_called_with("** no instance found **")
    ...
    def test_update_nonexisting_class(self):
        ...
        prompt = "(hbnb)"
        with patch('builtins.print') as mock_print:
            console.onecmd("update MyModel 1234-1234-1234 \
    ...
    def test_update_nonexising_instance(self):
        ...
        console = HBNBCommand()
        with patch('builtins.print') as mock_print:
            console.onecmd("update BaseModel 121212 \
                                     email 'airbnb@mail.com'")
            mock_print.assert_called_with("** no instance found **")
    ...
    def test_update_missing_attribute_name(self):
        ...
        with patch('builtins.print') as mock_print:
            id = str((BaseModel()).id)
            console.onecmd(f"update BaseModel {id}")
    ...
    def test_update_missing_attribute_value(self):
        ...
        with patch('builtins.print') as mock_print:
            id = str((BaseModel()).id)
            console.onecmd(f"update BaseModel {id} email")
