...
class TestState(unittest.TestCase):
    ...
    def test_instance_attributes(self):
        ...
        """
        self.assertTrue(hasattr(TestState.the_state, "name"))

    def test_default_attr_values(self):
        """tests the default set attribute values"""
        self.assertEqual(TestState.the_state.name, "")

    def test_state_print(self):
        """tests the format the state is printed in"""
        with patch('sys.stdout', new_callable=io.StringIO) as mock_stdout:
            print(TestState.the_state)
        expected_output = (
            "[State] ({}) ".format(TestState.the_state.id) +
            "{}".format(TestState.the_state.__dict__)
        )
