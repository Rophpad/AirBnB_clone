...
class TestReview(unittest.TestCase):
    ...
    def test_instance_attributes(self):
        ...
        for attribute in attributes_to_check:
            self.assertTrue(hasattr(TestReview.the_review, attribute))

    def test_review_print(self):
        """tests the format the review is printed in"""
        with patch('sys.stdout', new_callable=io.StringIO) as mock_stdout:
            print(TestReview.the_review)
        expected_output = (
            "[Review] ({}) ".format(TestReview.the_review.id) +
            "{}".format(TestReview.the_review.__dict__)
        )
    ...
    def test_default_attr_values(self):
        ...
        for attribute in attributes_to_check:
            default_val = ""
            self.assertEqual(getattr(TestReview.the_review, attribute),
                             default_val)
